...
export default function AssinmentResultTable({sorce}:{sorce:AssinmentResultInterface}) {
  return (
    <div className="w-full text-gray-700 border-black flex justify-center items-center">
         ...
         <div className="table w-full bg-green-200 border-1 border-blue-400">
              <div className="table-row-group">
                ...
                {
          sorce?.Assinments?.assinmentbody?.questions?.map((e,i)=>
            <>
            <div key={i} className={`${i % 2 == 0 ? 'bg-yellow-50  text-purple-600':'bg-blue-100 text-orange-700'} table-row-group `}>
                ...
                { return <div key={i} className="flex flex-col bg-transparent border-none">
                   {(JSON.parse(sorce?.answersbody as unknown as string)?.filter((ele: { id: string })=>
                    ele.id === e.id
              ...
              <div className="flex flex-col bg-transparent border-none justify-center items-center">{
                (JSON.parse(sorce?.answersbody as unknown as string)?.filter((ele: { id: string })=>
                  ele.id === e.id
                )[0]?.answer === e.answer ? <span className="text-green-700 gap-2 flex bg-transparent border-none justify-center items-center ">{JSON.parse(sorce?.answersbody  as unknown as string)?.filter((ele: { id: string })=>
                  ele.id === e.id
                )[0]?.answer }
                <icon.GiCheckMark  className="text-green-700 bg-transparent border-none"/>
                </span> : 
                <span className="text-red-700 gap-2 flex bg-transparent border-none justify-center items-center ">{JSON.parse(sorce?.answersbody  as unknown as string)?.filter((ele: { id: string })=>
                  ele.id === e.id
                )[0]?.answer }
              <icon.FaXmark  className="text-red-700 bg-transparent border-none"/>
                </span>)
