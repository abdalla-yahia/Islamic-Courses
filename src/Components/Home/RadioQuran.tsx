...
export default function RadioQuran() {
  ...
  return (
    ...
    <div onClick={()=>{setToggle(!toggle)}} className="w-full scrollbar-hide flex flex-row-reverse justify-between items-center my-1 rounded h-fit px-2 text-slate-100 shadow bg-blue-950">
    <input value={SearchText} onChange={(e)=>{SetSearchText(e.target.value);setToggle(true)}} type="search" name="" id="" className='m-2 px-2 z-40 outline-none rounded bg-blue-900 text-white w-[70%] self-center' placeholder=' بحث عن إذاعة ....' />
    <div  className="flex relative justify-start items-start w-full" dir='rtl'>
        ...
        {toggle && <div className='bg-slate-600 absolute left-0 top-[10px] w-[80%] z-40 max-h-[400px] scrollbar-hide overflow-y-scroll text-red-500 flex justify-start items-start flex-col'>
        {
            ...
            (
                ...
                <span className='text-2xl font-bold text-white border-b-2 border-red-600'>حفص عن عاصم الكوفي</span>
                {
                    Radios?.filter(el=>!el?.name.includes('رواي') && !el?.name.includes('باللغة') ).map((radio,index)=>
                        <span onClick={()=>{seturlRadio(radio?.url);setToggle(false);setRadioName(radio?.name);setplay(true)}} className='hover:bg-blue-500 hover:text-white px-2 py-1 cursor-pointer rounded' key={index} title={radio?.name}>{radio?.name}</span>)
                }
            ...
            <span onClick={()=>{seturlRadio(radio?.url);setToggle(false);setRadioName(radio?.name);setplay(true);SetSearchText('')}} className='hover:bg-blue-500 hover:text-white px-2 py-1 cursor-pointer rounded' key={index} title={radio?.name}>{radio?.name}</span>)
        }
        </div>}
    </div>
    </div>
    {/*Player Radio Box */}
    <div className="w-full flex justify-evenly items-center rounded h-5 px-2 py-1 bg-blue-950 shadow">
