...
export default function ExamResultTable({ ExamResult, Exam }: { ExamResult: ExamResultInterface,Exam:AllExamsInterface }) {
  const { DetailsToggle, setDetailsToggle } = ExamHook({ Exam });
  return (
    <>
      ...
      <div className="w-full text-gray-700 border-black flex justify-center items-center">
        <div className="table w-full bg-green-200 border-1 border-blue-400">
          <div className="table-row-group">
            ...
            <span className="border-1 border-blue-400  py-2  table-cell text-center  ">
              السؤال
            </span>
            <span className="border-1 border-blue-400  py-2  table-cell text-center  ">
              الإختيار
            ...
            <span className="border-1 border-blue-400  py-2 hidden md:table-cell lg:table-cell text-center ">
              {" "}
              درجة السؤال
            </span>
          </div>
          {ExamResult?.Exam?.Exambody?.questions?.map((e, i) => (
            <>
              <div
                key={i}
                className={`${
              ...
              >
                <span className="border-1 border-blue-400  hidden md:table-cell lg:table-cell text-center ">
                  {e.id}
                </span>
                ...
                <span className="border-1 border-blue-400  table-cell text-center ">
                  {e.choase.map((ch, i) => (
                    <span
                      key={i}
                      className="flex flex-col bg-transparent border-none"
                    >
                      {ch}
                    </span>
                  ))}
                ...
                <div className=" border-1 border-blue-400  table-cell text-center items-center ">
                  {ExamResult?.answersbody &&
                  JSON.parse(ExamResult?.answersbody).filter(
                    (ele: { id: string }) => ele.id === e.id
                  )[0]?.type === "checkbox" ? (
                    //If type is checkbox
                    <div className="bg-transparent border-none">
                      {JSON.parse(ExamResult?.answersbody).filter(
                          ...
                          {e?.answer?.map((ans, i) => {
                            return (
                              <div
                                ...
                                className="flex flex-col bg-transparent border-none"
                              >
                                {JSON.parse(ExamResult?.answersbody)
                                  ?.filter(
                                    (ele: { id: string }) => ele.id === e.id
                                  )[0]
                                  ?.answer?.includes(ans) ? (
                        ...
                        <div className="flex flex-col bg-transparent border-none justify-center items-center">
                          {JSON.parse(ExamResult?.answersbody)
                            .filter((ele: { id: string }) => ele.id === e.id)[0]
                            ?.answer.map(
                              (
                                it:
                                  | string
                                ...
                                return e?.answer?.includes(
                                  it as unknown as string
                                ) ? (
                                  <span
                                    key={i}
                                    className="text-green-700 flex bg-transparent border-none justify-center mb-2 gap-2  items-center "
                                  >
                                    {it}
                                    <icon.GiCheckMark className="text-green-700 bg-transparent border-none" />
                                  </span>
                                  ...
                                  <span
                                    key={i}
                                    className="text-gray-600 flex bg-transparent border-none justify-center mb-2 gap-2  items-center "
                                  >
                                    {it}
                    ...
                    <div className="flex flex-col bg-transparent border-none justify-center items-center">
                      {JSON.parse(ExamResult?.answersbody).filter(
                        ...
                        <span className="text-green-700 gap-2 flex bg-transparent border-none justify-center items-center ">
                          {
                            JSON.parse(ExamResult?.answersbody).filter(
                              (ele: { id: string }) => ele.id === e.id
                            )[0]?.answer
                        ...
                        <span className="text-red-700 gap-2 flex bg-transparent border-none justify-center items-center ">
                          {
                            JSON.parse(ExamResult?.answersbody).filter(
                              (ele: { id: string }) => ele.id === e.id
                            )[0]?.answer
