...
const UPLOAD_IMAGES_DIR = path.join(process.cwd(), "public/uploads", "images");
const UPLOAD_FILES_DIR = path.join(process.cwd(), "public/uploads", "files");
const UPLOAD_AUDIOS_DIR = path.join(process.cwd(), "public/uploads", "audios");
const UPLOAD_VEDIOS_DIR = path.join(process.cwd(), "public/uploads", "videos");
          ...
          if(Extention === '.mp3' || Extention === '.wav' || Extention === '.ogg' || Extention === '.flac'){
            const buffer = Buffer.from(await file.arrayBuffer());
              if (!fs.existsSync(UPLOAD_AUDIOS_DIR)) {
                fs.mkdirSync(UPLOAD_AUDIOS_DIR, { recursive: true });
            }
            fs.writeFileSync(
          ...
          if(Extention === '.mp4' || Extention === '.avi' || Extention === '.mov' || Extention === '.flv'){
            const buffer = Buffer.from(await file.arrayBuffer());
              if (!fs.existsSync(UPLOAD_VEDIOS_DIR)) {
                fs.mkdirSync(UPLOAD_VEDIOS_DIR, { recursive: true });
            }
            fs.writeFileSync(
