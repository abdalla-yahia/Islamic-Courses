...
export default function RadioQuran() {
  ...
  return (
    ...
    <div  className="flex relative justify-start items-start w-full" dir='rtl'>
        ...
        {toggle && <div className='bg-[#eea127] rounded-md absolute left-0 top-[100%] w-[80%] z-40 max-h-[400px] scrollbar-hide overflow-y-scroll text-red-500 flex justify-start items-start flex-col'>
        {
            Radios?.length > 0 && SearchText === '' ?
            (
                <>
                <span onClick={()=>{seturlRadio('https://stream.radiojar.com/8s5u5tpdtwzuv');setToggle(false);setRadioName('إذاعة القرآن الكريم من القاهرة');setplay(true)}} className='hover:bg-[#9f6301] text-white hover:text-white px-2 py-1 cursor-pointer rounded'  title={'إذاعة القرآن الكريم من القاهرة'}>{'إذاعة القرآن الكريم من القاهرة'}</span>
                <span className='text-2xl font-bold text-white border-b-2 border-red-600'>حفص عن عاصم الكوفي</span>
                {
                    Radios?.filter(el=>!el?.name.includes('رواي') && !el?.name.includes('باللغة') ).map((radio,index)=>
                        <span onClick={()=>{seturlRadio(radio?.url);setToggle(false);setRadioName(radio?.name);setplay(true)}} className='hover:bg-[#9f6301] text-white hover:text-white px-2 py-1 cursor-pointer rounded' key={index} title={radio?.name}>{radio?.name}</span>)
                }
            ...
            <span onClick={()=>{seturlRadio(radio?.url);setToggle(false);setRadioName(radio?.name);setplay(true);SetSearchText('')}} className='hover:bg-[#9f6301] text-white hover:text-white px-2 py-1 cursor-pointer rounded' key={index} title={radio?.name}>{radio?.name}</span>)
        }
        </div>}
    </div>
    </div>
